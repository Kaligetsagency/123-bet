...
export default function App() {
  const [activeTab, setActiveTab] = useState('home'); // home, play, profile
...
function LandingView({ setActiveTab }) {
  return (
    <div className="p-6 max-w-md mx-auto pt-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-3xl p-6 mb-8 shadow-xl">
      ...
      <div className="bg-gradient-to-br from-indigo-900/40 to-purple-900/40 border border-indigo-500/30 rounded-3xl p-6 shadow-xl mb-8">
...
function GameEngine({ balance, setBalance }) {
  ...
  return (
    <div className="p-6 max-w-md mx-auto pt-8 h-full flex flex-col animate-in fade-in zoom-in-95 duration-300">
      <div className="flex justify-between items-center mb-8 bg-slate-800/80 p-3 rounded-2xl border border-slate-700/50">
      ...
      <div className="flex-1 flex flex-col items-center justify-center w-full">
        {gameState === 'lobby' && (
          <div className="w-full flex flex-col items-center">
            ...
            <h2 className="text-2xl font-bold mb-2">Find Opponent</h2>
            <p className="text-slate-400 mb-8">Stake: TZS 1,000 | Win: TZS 1,800</p>
        ...
        {gameState === 'playing' && (
          <div className="w-full flex flex-col items-center w-full">
            ...
            <p className="mb-6 text-slate-400 uppercase tracking-widest font-bold">Make Your Choice</p>
        ...
        {gameState === 'result' && (
          <div className="w-full text-center flex flex-col items-center animate-in zoom-in duration-300">
            <h2 className={`text-6xl font-black mb-10 tracking-tighter drop-shadow-lg ${result === 'WIN' ? 'text-green-400' : result === 'TIE' ? 'text-yellow-400' : 'text-red-500'}`}>
              {result}
            </h2>
...
function ProfileView({ balance, setIsLoggedIn }) {
  return (
    <div className="p-6 max-w-md mx-auto pt-12 animate-in fade-in slide-in-from-right-4 duration-300">
      {/* Profile Header */}
      ...
      {/* Balance Card */}
      <div className="bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-700 p-6 rounded-3xl mb-8 shadow-2xl relative overflow-hidden">
        <Wallet className="absolute -bottom-4 -right-4 w-32 h-32 text-slate-700/30 rotate-12" />
        <p className="text-slate-400 text-sm font-semibold tracking-wider uppercase mb-1">Available Balance</p>
        <h3 className="text-4xl font-mono font-black text-white mb-6">TZS {balance.toLocaleString()}</h3>
      ...
      {/* Settings Menu */}
      <div className="bg-slate-800/50 border border-slate-700/50 rounded-3xl overflow-hidden mb-8">
...
function LoginScreen({ setIsLoggedIn }) {
  return (
    <div className="h-screen flex flex-col items-center justify-center p-6 max-w-md mx-auto text-center">
      ...
      <h1 className="text-4xl font-black mb-2">123 BET</h1>
      <p className="text-slate-400 mb-12">Login to enter the arena.</p>
      
      <div className="w-full space-y-4">
        <input type="text" placeholder="Phone Number" className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-4 text-white placeholder-slate-500 focus:outline-none focus:border-blue-500" />
        <input type="password" placeholder="PIN" className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-4 text-white placeholder-slate-500 focus:outline-none focus:border-blue-500" />
        <button onClick={() => setIsLoggedIn(true)} className="w-full bg-gradient-to-r from-blue-500 to-cyan-500 text-white font-black text-lg py-4 rounded-xl shadow-lg mt-4 active:scale-95 transition-all">
          SECURE LOGIN
        </button>
      </div>
    </div>
